...
@Injectable()
export class CompaniesUploadService {
  private readonly logger = new Logger(CompaniesUploadService.name);
  private readonly uploadDir: string;

  constructor(private readonly config: ConfigService) {
    this.uploadDir = join(process.cwd(), 'uploads', 'companies');
  ...
  async saveDataUrl(dataUrl: string) {
    ...
    const url = `/uploads/companies/${safeName}`;
  ...
  private async compressBuffer(
    ...
    try {
      ...
      this.logger.warn('Company image compression failed, saving original', e);
